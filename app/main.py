import logging
import concurrent.futures
from fastapi import FastAPI, HTTPException, Request
from dotenv import load_dotenv
import os
from app.services.fetch_news_2 import (
    setup_session_with_proxy,
    fetch_symbols_from_mongo,
    fetch_and_save_news,
)
from app.services.fetch_and_save_symbols_1 import fetch_and_insert_symbols
from app.services.data_processing_3 import fetch_process_save_news_items
from app.services.fetch_price_4 import calculate_and_save_predicted_prices
from app.services.fetch_gap_price_5 import fetch_and_save_symbols

# Load environment variables from .env file
load_dotenv()

# Set up logging
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s - %(asctime)s - %(message)s"
)
logging.getLogger().disabled = False

# Initialize FastAPI
app = FastAPI()


@app.get("/")
async def check_ready():
    return {"message": "Ok"}


@app.get("/1")
async def fetch_and_save_symbols_endpoint():
    try:
        fetch_and_insert_symbols()
        return {"message": "Symbols fetched and saved successfully"}
    except Exception as e:
        logging.error(f"An error occurred: {e}")
        raise HTTPException(
            status_code=500, detail="An error occurred while processing the request"
        )


@app.get("/2")
async def fetch_and_save_news_endpoint(request: Request):
    try:
        proxy_enabled = (
            request.query_params.get("proxy_enabled", "false").lower() == "true"
        )
        proxy = request.query_params.get("proxy", None)
        pool_maxsize = int(request.query_params.get("pool_maxsize", 20))

        session = setup_session_with_proxy(
            proxy_enabled=proxy_enabled, proxy=proxy, pool_maxsize=pool_maxsize
        )
        symbols = fetch_symbols_from_mongo()

        if not symbols:
            logging.error("No symbols fetched from MongoDB")
            raise HTTPException(
                status_code=500, detail="No symbols fetched from MongoDB"
            )
        else:
            max_workers = 10

            with concurrent.futures.ThreadPoolExecutor(
                max_workers=max_workers
            ) as executor:
                futures = [
                    executor.submit(fetch_and_save_news, session, symbol)
                    for symbol in symbols
                ]
                for future in concurrent.futures.as_completed(futures):
                    try:
                        future.result()
                    except Exception as e:
                        logging.error(f"An error occurred: {e}")

            return {"message": "News fetched and saved successfully"}

    except Exception as e:
        logging.error(f"An error occurred: {e}")
        raise HTTPException(
            status_code=500, detail="An error occurred while processing the request"
        )


@app.get("/3")
async def fetch_process_save_news_items_endpoint():
    try:
        fetch_process_save_news_items()
        return {"message": "News items fetched, processed, and saved successfully"}
    except Exception as e:
        logging.error(f"An error occurred: {e}")
        raise HTTPException(
            status_code=500, detail="An error occurred while processing the request"
        )


@app.get("/4")
async def calculate_and_save_predicted_prices_endpoint():
    try:
        calculate_and_save_predicted_prices()
        return {"message": "Predicted prices calculated and saved successfully"}
    except Exception as e:
        logging.error(f"An error occurred: {e}")
        raise HTTPException(
            status_code=500, detail="An error occurred while processing the request"
        )


@app.get("/5")
async def fetch_and_save_last_prices_endpoint():
    try:
        fetch_and_save_symbols()
        return {"message": "Last prices fetched and saved successfully"}
    except Exception as e:
        logging.error(f"An error occurred: {e}")
        raise HTTPException(
            status_code=500, detail="An error occurred while processing the request"
        )


# Run the application
if __name__ == "__main__":
    import uvicorn

    port = int(os.getenv("PORT", 8000))
    uvicorn.run(app, host="0.0.0.0", port=port, log_level="info")
